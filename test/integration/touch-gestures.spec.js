...
test.describe('Touch gestures', () => {
  ...
  test('double-tap on a cell enters edit mode', async ({ page }) => {
    ...
    // Tap 2 (within DOUBLE_TAP_WINDOW_MS)
  ...
  test('single tap on a cell does not enter edit mode', async ({ page }) => {
    const cell = page.locator('.subwindow table tbody td.data-cell').first();
    const handle = await cell.elementHandle();
    const { x, y } = await centerOf(cell);
  ...
  test('long-press on a cell does not enter edit mode', async ({ page }) => {
    // Long-press used to enter edit mode; now the gesture is double-tap only.
    const cell = page.locator('.subwindow table tbody td.data-cell').first();
    const handle = await cell.elementHandle();
    const { x, y } = await centerOf(cell);
  ...
  test('second tap with pan becomes drag-select, not edit', async ({ page }) => {
    const cell = page.locator('.subwindow table tbody td.data-cell').first();
    const handle = await cell.elementHandle();
    const { x, y } = await centerOf(cell);
    ...
    await page.waitForTimeout(50);
    await dispatchTouch(page, handle, 'touchend', x, y);
    // Tap 2 with pan — exceeds the 5 px threshold so it's treated as a drag
    await page.waitForTimeout(100);
    await dispatchTouch(page, handle, 'touchstart', x, y);
