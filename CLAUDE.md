...
- Touch gestures are layered on top of the mouse handlers via separate `touchstart`/`touchmove`/`touchend` listeners. State lives at IIFE scope: `_touchLongPress` (cell long-press → edit mode after `LONG_PRESS_MS` = 600 ms), `_touchHeaderDrag` (active header drag), `_lastHeaderTap` (tracks the first tap of a 1.5-tap drag by table + column name, since `rebuildTable` recreates `th` elements between taps). `enterEditMode` must be called synchronously inside the `touchend` user gesture so iOS/Android open the virtual keyboard. The header drag sets `th._didDrag = true` so the trailing click doesn't sort
