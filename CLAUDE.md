...
**Tabbing:** Hold Shift and drag a window's titlebar onto another window's titlebar area to merge them into a tab group. Tabs replace the titlebar, spanning the full width — one tab per window. Only 1 level of tabbing (tab groups cannot be nested inside tab groups, but each pane in a dock split can be its own tab group). The active tab's `.win-body` and `.win-statusbar` are visible; inactive tabs' content is hidden via `display: none` with `dataset.winId` for identification. Double-clicking a tab toggles maximize/restore on the dock container. Single-tab leaves inside a split hide their tab bar (class `single-tab`) but the tab stretches to fill width via `flex: 1`; the tab bar reappears when a second tab is added.
