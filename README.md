...
- **Row and column management** — Right-click (or long-press) a row number to insert below or delete. Right-click (or long-press) a column header to rename, sort, insert a column to the right, or delete. Right-click the `#` corner cell to insert a row or column when the table is empty, or to rename the table. Ctrl/Cmd+click a column header to rename inline (duplicate names are rejected with a red border). Full undo/redo for all structural operations. Columns open auto-fitted to their content; resize any column by dragging column borders — including the row-number column (double-click to auto-fit based on all rows, not just visible ones). When multiple columns are selected, double-click auto-fits all selected columns and drag-resize sets all selected columns to the same width
...
- **Quick search** — The table toolbar's default Search mode (press `/` in an active table window to focus it) highlights every matching cell — column headers included — without filtering rows; flip the toggle to Filter for SQL WHERE filtering. Matches formatted display values when plugin formatting is enabled, raw values otherwise. Enter or Tab moves the cursor into the first match, Tab/Shift+Tab (or Enter/Shift+Enter) step through matches, Escape returns to the search box; `n`/`p` jump to the next/previous match from anywhere in the table (`N`/`P` reverse them: `N` previous, `P` next). A match counter and a toast make it obvious when the search wraps around. A **Clear** button (shown while the box has text) empties the search or filter in one click
- **Column numbers** — The right side of the status bar shows the selected column's position (`Col 3 of 12`, or `Cols 2–4 of 12` for a multi-column selection) in place of the column count. View > Show Column Numbers adds a numbered line above every column header; the setting is remembered
- **Selection statistics** — Selecting two or more cells shows Count, Sum, Avg, Min, and Max on the right side of the status bar, next to the column position (numeric stats computed over the numeric cells in the selection)
...
- **Resize columns** — Columns auto-fit to their content when a table first opens (very wide columns are capped at 75% of the window). Drag any column border to resize — the resize handle spans both sides of the divider line, including the `#` row-number column. Double-click to auto-fit the column to its content (measures all rows, not just visible ones). When multiple columns are selected (e.g. via Ctrl+A), double-click auto-fits all selected columns, and drag-resize sets all selected columns to the dragged column's width on release
- **Column numbers** — Selecting a cell or column shows its position (`Col 3 of 12`) on the right side of the status bar. Turn on View > Show Column Numbers to number every column header; numbers follow the current column order, which is also the order used on save
- **Rename tables** — Ctrl/Cmd+click the window title, use Edit > Rename Table…, or right-click (or long-press) the `#` corner cell
...
- **Select rows** — Tap a row number, or pan down the row-number column for a range
- **Context menus** — Long-press (hold ~half a second) a row number, column header, or the `#` corner cell for the same menu as a right-click: insert/delete rows and columns, rename a column or the table, and sort
...
- **Dock a window** — Long-press the title bar (no prior tap) until the device vibrates, then drag onto another window — the touch equivalent of Shift+drag; drop on a title bar to tab, on a body region to split
- **Resize** — Drag window edges/corners, column borders, dock splitters, and the console divider directly. Double-tap a column border to auto-fit, a splitter to reset the split, or a tab to maximize the dock
- **Undock a tab** — Drag a tab vertically out of the tab bar and drop it on another window or empty space
- Touch devices get larger hit targets and always-visible controls automatically; on phone-sized screens new windows open maximized

<!-- ![Inline editing](screenshots/editing.png) -->

### Sorting and Filtering

- **Sort** — Click the sort badge (triangle icon) in a column header to cycle: ascending → descending → unsorted. Numeric values sort numerically (exact ordering even for integers beyond 2⁵³, so large IDs keep their true order); other values sort alphabetically
...
- **Filter** — Flip the toolbar toggle to **Filter** and type a SQL WHERE expression in the filter bar (without the `WHERE` keyword):
  ...
  ```
  A **Clear** button appears while the bar has text and removes the filter, restoring all rows
- **Quick search (default)** — With the toolbar toggle on **Search** (the default; `/` also focuses it whenever the table window is active), type to highlight every matching cell, using the same highlight as Find & Replace; rows are not filtered. Column headers are searched too, and header matches come before the data cells when stepping through. The search matches what you see: formatted display values for columns with an enabled plugin format, raw values otherwise (Find & Replace always matches raw values). A match counter next to the input shows your position. Enter or Tab moves the cursor into the first match; Tab/Shift+Tab (or Enter/Shift+Enter) then step to the next/previous match and Escape returns to the search box. While stepping through matches, any other key acts on the matched cell as usual and ends the match navigation — `i`/F2/Ctrl+U edit it, arrows move the selection, Shift+arrows extend it — while `n`/`p` keep jumping to the next/previous match from anywhere in the table (`N`/`P` go the other way: `N` previous, `P` next). Off-screen matches scroll smoothly into view (respects the OS reduced-motion setting). Wrapping past the last (or first) match shows "wrapped" in the counter and a toast. Pressing `/` while the toggle is on Filter switches to search mode only temporarily — Escape flips the toggle back to Filter, keeping the highlights until the search is cleared (empty the input — the **Clear** button next to it does this in one click — or Escape in the table)
...
The SQL Console at the bottom runs queries against all open tables using SQLite syntax. Press **Ctrl+Enter** (Cmd+Enter on Mac) to execute. The console and filter inputs feature SQL syntax highlighting. Query results open as new queryable tables. A query that matches no rows opens no result table — the console status line says so in amber, the color used for notable-but-non-error outcomes (errors stay red).

**Autocompletion:** As you type, a dropdown suggests table names (after `FROM`, `JOIN`, `INSERT INTO`, etc.), column names (after `tablename.` or a query alias, plus columns of any table referenced in the statement), and SQL keywords — filtered to those valid at the cursor position (for example, type names appear only inside `CAST(... AS`, and commands like `VACUUM` only at the start of a statement). **Tab** or **Enter** accepts the highlighted suggestion, **↑**/**↓** navigate, **Escape** dismisses, and **Ctrl+Space** opens the dropdown manually. Names that need quoting are inserted bracket-quoted automatically. Suggestions never appear inside string literals or comments, and while the dropdown is closed no keys are intercepted. The same autocompletion works in every table's filter bar, where that table's own columns are suggested first.
...
- **Tab windows** — Hold Shift and drag a window onto another window's title bar to merge them into a tab group (on touch: long-press the title bar, then drag). Click tabs to switch between windows
- **Split dock** — Hold Shift and drag a window onto the body area of another window (on touch: long-press the title bar, then drag). Drop zones are divided diagonally — drop on the top, right, bottom, or left region to split that direction
...
| Tab / Shift+Tab or Enter / Shift+Enter (match navigation) | Step to the next / previous quick-search match; any other key ends navigation and acts normally |
...
| N / P (cell selected, quick search active) | Jump to the previous / next quick-search match |
