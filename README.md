...
- **Column badges** — Small colored badges appear at the top-right corner of column headers to indicate active features. Badge order (left to right): linked (teal dot), formatted (pink dot), filtered (green dot), sorted (gold triangle with direction). Badge colors match the status bar chips. All four badge slots are always reserved so badges don't shift when others appear or disappear. Suspended features hide their badge without shifting neighbors
